...
- **Stack Selection:** Next.js 15 (App Router) + TypeScript + Vanilla CSS / CSS Modules with bespoke CSS variables directly reflecting YarshaByte's live design tokens (`#f5efe7` sand canvas, `#1e1a18` foreground, `#b86a2c` terracotta accent, `#33202a` deep navy).
- **Routing Strategy:**
...
- Create `src/styles/tokens.css` with exact YarshaByte color palette, typography tokens, outer desktop frame, and noise texture.
...
- `RoleSection`: `02 — MY ROLE AT YARSHABYTE`, categorized responsibilities, domain competencies (no percentage bars), milestones.
- `WorkSection`: `03 — WORK / CONTRIBUTIONS`, real YarshaByte project showcases, role tags, external links.
...
- `Footer`: Editorial footer anchored in YarshaByte identity.
